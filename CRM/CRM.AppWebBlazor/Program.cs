using CRM.AppWebBlazer.Data; // Importa el espacio de nombres donde se encuentra CustomerService
...
// Configura y agrega un cliente HTTP con nombre "CRMAPI" builder.
...
c.BaseAddress = new Uri(builder.Configuration["UrlsAPI:CRM"]);
    // Puedes configurar otras opciones del HttpClient aqui según sea necesario
    });
...
if (!app.Environment.IsDevelopment())
{
    ...
    app.MapBlazorHub(); // Mapea BlazorHub para habilitar la funcionalidad de Blazor en tiempo real
                        app.MapFallbackToPage("/_Host"); // Mapea la página Host para servir aplicaciones Blazor
    app.Run();
